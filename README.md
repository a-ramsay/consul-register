...
There are four configuration options available via environment variables:
...
-   `AUTO_REFRESH`: (Default: `true`) This can be set to `false` to disable refreshing the services every 10 seconds. The auto refresh exists to clean up any edge cases that can appear when the Docker service restarts (e.g. on reboot) and services start in an unpredictable order.
